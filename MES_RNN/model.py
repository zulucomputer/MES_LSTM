    ...
    def es(self, df):
        ...
        if isdir(self.params_path) == False:
            ...
            for col in df.columns.to_list():
                ...
                vars()["param_df_{}".format(ind)].to_pickle("params/" + "param_df_{}".format(ind) + ".pkl")
                params_dict["param_df_{}".format(ind)] = vars()["param_df_{}".format(ind)]
        else:
            # load params
            chdir(self.params_path)
            for file in glob("*df*"):
        ...
        if isdir(self.internals_path) == False:
            mkdir(self.internals_path)
            ...
            for col in df.columns.to_list():
                ...
                vars()["int_df_{}".format(ind)].to_pickle(self.internals_path + "/int_df_{}".format(ind) + ".pkl")
                internals_dict["int_df_{}".format(ind)] = vars()["int_df_{}".format(ind)]
        else:
            # load internals
            chdir(self.internals_path + "/")
            for file in glob("*df*"):
                print("...loading DataFrame - {}".format(splitext(file)[0]))
                vars()[splitext(file)[0]] = pd.read_pickle(file)
...
class rnn():
    ...
    def pi_model(self, test, x_train, y_train, x_valid, y_valid, x_test):
        ...
        model = Sequential()
        model.add(LSTM(self.lstm_size, activation = self.activation, input_shape = (self.n_input_train, n_features_train)))
        model.add(DenseFlipout(len(self.y_col)))
        model.compile(optimizer = self.optimizer, loss = self.loss)
        print(model.summary())
